...
void PoolAllocator<block_size, num_blocks>::StartUp()
{
	...
	m_pFreeList = m_pPool;		// The free list starts at the first block

	// Connect the blocks to each other
	for (int i = 0; i < num_blocks; i++)
	{
		if (i < num_blocks - 1)		// The last block doesn't need to be connected
		{
			m_pPool[i]._next = &m_pPool[i + 1];
		}
#ifdef _DEBUG
		m_pPool[i]._boundary = 0xdeadbeef;
		for (int j = 0; j < block_size; j++)	// set all of the memory to 0xde
		{
...
{
	Dbg_Assert(size <= block_size, "Size must be less than block size!");
	Dbg_Assert(m_pFreeList, "There are no more blocks free!");
	if (!m_pFreeList) {
		return 0;
	}

	PoolBlock<block_size>* free_block = m_pFreeList;
	m_pFreeList = m_pFreeList->_next;	// Update the free list pointer to the next free block

	iBlocksFree--;

	return free_block->_memory;
}
...
{
	PoolBlock<block_size>* block = static_cast<PoolBlock<block_size>*>(ptr);

#ifdef _DEBUG
	Dbg_Assert(block->_boundary == 0xdeadbeef, "Bounds were overwritten!");
