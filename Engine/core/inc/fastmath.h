...
public:
	...
	FastMatrix4 GetTranspose() const
	{
		__m128 rhs_row0 = _rows[0];
		__m128 rhs_row1 = _rows[1];
		__m128 rhs_row2 = _rows[2];
		__m128 rhs_row3 = _rows[3];

		// transpose the rhs matrix
		_MM_TRANSPOSE4_PS(rhs_row0, rhs_row1, rhs_row2, rhs_row3);

		FastMatrix4 result;
		result._rows[0] = rhs_row0;
		result._rows[1] = rhs_row1;
		result._rows[2] = rhs_row2;
		result._rows[3] = rhs_row3;
		return result;
	}
	...
	void Multiply(const FastMatrix4& rhs)
	{
		...
		for (int i = 0; i < 4; i++) {
			// Take the dot product of each row in the matrix
			__m128 xprime = _mm_dp_ps(_rows[i], rhs_row0, 0xf1);
			__m128 yprime = _mm_dp_ps(_rows[i], rhs_row1, 0xf2);
			__m128 zprime = _mm_dp_ps(_rows[i], rhs_row2, 0xf4);
			__m128 wprime = _mm_dp_ps(_rows[i], rhs_row3, 0xf8);

			// Accumulate the dot products into one vector
			__m128 result = _mm_add_ps(xprime, yprime);
			result = _mm_add_ps(result, zprime);
			result = _mm_add_ps(result, wprime);
...
public:
	...
	{
		__m128 temp = _mm_dp_ps(_data, rhs._data, 0x77);
	...
	{
		__m128 temp;
		temp = _mm_dp_ps(_data, _data, 0x77);	// Dot the vector with itself
		temp = _mm_rsqrt_ps(temp);		// Take the inverse reciprocal
		_data = _mm_mul_ps(_data, temp);	// Multiply this by the original vector
 	}
	...
	{
		__m128 tempA = _mm_shuffle_ps(lhs._data, lhs._data, _MM_SHUFFLER(1, 2, 0, 0));	// shuffle A to <Ay, Az, Ax>
		__m128 tempB = _mm_shuffle_ps(rhs._data, rhs._data, _MM_SHUFFLER(2, 0, 1, 0));	// shuffle B to <Bz, Bx, By>
		__m128 result = _mm_mul_ps(tempA, tempB);	// tempA * tempB
		tempA = _mm_shuffle_ps(lhs._data, lhs._data, _MM_SHUFFLER(2, 0, 1, 0));	// shuffle A to <Az, Ax, Ay>
		tempB = _mm_shuffle_ps(rhs._data, rhs._data, _MM_SHUFFLER(1, 2, 0, 0));	// shuffle B to <By, Bz, Bx>
		tempA = _mm_mul_ps(tempA, tempB);	// tempA * tempB
		result = _mm_sub_ps(result, tempA);	// -= tempA;
	...
	{
		__m128 tempA = _mm_setr_ps(1.0f - f, 1.0f - f, 1.0f - f, 0);
		tempA = _mm_mul_ps(tempA, a._data);
		__m128 tempB = _mm_setr_ps(f, f, f, 0);
		tempB = _mm_mul_ps(tempB, b._data);
		tempB = _mm_add_ps(tempA, tempB);
		return FastVector3(tempB);
	}
											...
											float fa, float fb, float fc)
	{
		...
		__m128 tempA = _mm_setr_ps(fa, fa, fa, 0.0f);
		__m128 tempB = _mm_setr_ps(fb, fb, fb, 0.0f);
		__m128 tempC = _mm_setr_ps(fc, fc, fc, 0.0f);
		__m128 tempD = _mm_setr_ps(fd, fd, fd, 0.0f);
		tempA = _mm_mul_ps(tempA, a._data);
		tempB = _mm_mul_ps(tempB, b._data);
		tempC = _mm_mul_ps(tempC, c._data);
		tempD = _mm_mul_ps(tempD, d._data);
		tempA = _mm_add_ps(tempA, tempB);
		tempA = _mm_add_ps(tempA, tempC);
		tempA = _mm_add_ps(tempA, tempD);
		return FastVector3(tempA);
	}
	...
	{
		// Set w of _data to 1.f
		__m128 one = _mm_set_ss(1.f);
		_data = _mm_insert_ps(_data, one, 0x30);

		// Take the dot product of each row in the matrix
		__m128 xprime = _mm_dp_ps(mat._rows[0], _data, 0xf1);
		__m128 yprime = _mm_dp_ps(mat._rows[1], _data, 0xf2);
		__m128 zprime = _mm_dp_ps(mat._rows[2], _data, 0xf4);
		__m128 wprime = _mm_dp_ps(mat._rows[3], _data, 0xf8);
	...
	{
		// Set w of _data to 0.f
		__m128 zero = _mm_set_ss(0.f);
		_data = _mm_insert_ps(_data, zero, 0x30);
...
public:
	...
	// Construct a Quaternion given Euler rotations
	static FastQuaternion FromEulerAngles(float pitch, float yaw, float roll)
	{
		FastQuaternion qx(FastVector3::UnitX, pitch);
		FastQuaternion qy(FastVector3::UnitY, yaw);
		FastQuaternion qz(FastVector3::UnitZ, roll);
		qx.Multiply(qy);
		qx.Multiply(qz);
		return qx;
	}
